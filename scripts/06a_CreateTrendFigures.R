...
# Figure made 2024-07-30 by Carol and Linnea ####

trend_data <- data |>
  filter(lagoslakeid != '454129') |># removed 454129 Lake Havasu 
  mutate(lake_namelagos=factor(lake_namelagos, levels=c('Chatfield Lake', 'McClure Lake', 'Tongue River Reservoir', 'Black Lake', 'Dillon Reservoir', 'Stampede Reservoir', 'Evergreen Lake', 'Bull Run Reservoir No. 2', 'Imperial Reservoir', 'Moses Lake', 'Potholes Reservoir'))) |>
...
ggplot(trend_data |>
         mutate(nutrient = ifelse(nutrient == 'TP', 'TP*100', 'TN'))) +
  ...
  theme(
    legend.position = c(0.85, 0.18)) + scale_y_continuous(labels = scales::scientific) 

ggsave('figures/Fig3_new.png', height=6.5, width=8.5, units='in', dpi=1200)
...
# Previous figure panels and drafts ####
