...
for(site_no in candidate_sites$sites){
  ...
  file_out <- paste0("data/results_TN/", site_no, "/loadflex.csv")
  if(!file.exists(file_out)){
    
    file_in <- paste0("data/nwis_TN/", site_no, ".rds")
    dt      <- readRDS(file_in)
    ...
    # save preds_annual as data/results_TN/{{site_no}}/loadflex.csv
    dir.create(paste0("data/results_TN/", site_no), showWarnings = FALSE, recursive = TRUE)
    write.csv(preds_annual,
              paste0("data/results_TN/", site_no, "/loadflex.csv"),
              row.names = FALSE)
  }
...
# sapply(candidate_sites, function(x) unlink(paste0("data/results_TN/", x, "/loadflex.csv")))
